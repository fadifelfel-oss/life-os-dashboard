...
// Patched 2026-07-15 (biweekly refresh): 'lyria-3-pro-preview' was the
// 'ingest' job's budget pick — it resolves against the live catalog (so it
// LOOKS valid) but Google Lyria 3 is a music/audio-generation model, not a
// text model. It was silently winning the match (listed first) over the
// real free text option, meaning a wrong-type model could get routed for
// summarizing web clippings/PDFs. Same failure class as owl-alpha: a
// technically-real ID doing the wrong job. Removed; nemotron-3-ultra-550b
// (confirmed free, 1M ctx, real text model) is now the resolved budget pick.
// Also added qwen3-coder:free to the 'code' budget pick — per July 2026
// community consensus it's now the strongest free coding model on
// OpenRouter (1M ctx vs north-mini-code's 256K); north-mini-code kept as
// fallback in the same match array.
// ============================================================
window.LIFEOS_JOBS = [
  {
    ...
    picks:[
      ...
      { role:'budget', match:['nemotron-3-ultra-550b','gpt-oss-20b','ling-2.6-flash'], fb:{name:'Nemotron 3 Ultra 550B (free)',provider:'NVIDIA',pp:0,pc:0,ctx:1000000}, why:'Free, 1M context — use for huge batches of light clippings where volume beats nuance.' },
  ...
  },
  {
    ...
    picks:[
      ...
      { role:'budget', match:['qwen3-coder:free','qwen3-coder','north-mini-code:free','north-mini-code'], fb:{name:'Qwen3 Coder 480B (free)',provider:'Qwen',pp:0,pc:0,ctx:1048576}, why:'Free, 1M context — the strongest free coding model on OpenRouter right now, ahead of North Mini Code on both capability and context size. Great for scaffolding, refactors, and glue code before a premium pass.' },
